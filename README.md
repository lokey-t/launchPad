...
从 [GitHub Releases](https://github.com/lokey-t/launchPad/releases/tag/v0.95) 下载：

| 发行包 | 说明 |
| --- | --- |
| `LaunchPad-v0.95-win-x64.zip` | 推荐。包含 .NET 运行时，解压后直接运行 `LaunchPad.exe`。 |
| `LaunchPad-v0.95-win-x64-framework-dependent.zip` | 精简包，需要 .NET 8 Desktop Runtime（x64）。解压后保留整个文件夹运行。 |
...
- **快捷键**：全局呼出和分类快捷键集中管理。分类行直接显示快捷键，修改与删除按钮并排。
- **界面动效**：关闭、快速、均衡、优化四种模式；连续文件夹变形过渡、细圆角滚动条和滚轮缓动。支持图标尺寸、窗口位置及自动隐藏设置。
