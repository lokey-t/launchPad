...
- Build all assets with `./tools/Build-Release.ps1 -Version VERSION -Dotnet PATH`. Requires Windows, .NET 8 SDK, PowerShell with Get-FileHash and NSIS 3 (optionally pass -MakeNsis PATH); no Python packages. Output must be fresh. If an old build exists, inspect and move that exact output aside within bin before rebuilding; never recursively delete computed paths.
- The script generates full portable ZIPs, an NSIS offline setup EXE and checksums under bin/releases/vVERSION. For bundled file-level deltas, pass -BaseSelfContainedDirectory OLD_SELF and -BaseFrameworkDependentDirectory OLD_FDD using clean, verified older publishes of the same architecture/flavor. Each supplied baseline adds one changed/new-file ZIP and one Format=2 bundle manifest; omitted baselines produce full-package-only releases. Keep complete multi-file ZIP layouts. Do not publish old Format=1 manifest names or per-file .gz assets: v0.97 migrates through the full ZIP fallback, and future clients use bundled deltas. Retain full ZIPs and checksums for unsupported bases or damaged installations. Read docs/UPDATES.md for the schema and run BundlePackagingTests.ps1 after Debug UpdateTests.
...
7. Confirm public release state, tag commit, three end-user packages, any generated bundle manifests and all referenced assets. Check application update discovery against the public release. Report commit and both release links, and any actual limitations.
