...
public sealed partial class ThemeCenterWindow : Window
{
    private readonly App _app;
    private readonly ComboBox _scope = new() { MinHeight = 34 };
    private readonly CheckBox _colors = new() { Content = AppLanguage.T("为此分类单独设置配色"), Margin = new Thickness(0,8,0,8) };
    private readonly CheckBox _background = new() { Content = AppLanguage.T("为此分类单独设置背景"), Margin = new Thickness(0,8,0,8) };
    ...
    private readonly ComboBox _base = new() { ItemsSource = new[] { AppLanguage.T("浅色控件"), AppLanguage.T("深色控件") }, MinHeight = 30 };
    private readonly WrapPanel _materialOptions = new();
    private readonly Dictionary<string,Button> _materialButtons = new();
    private readonly CheckBox _materialOverride = new() { Content=AppLanguage.T("为此分类单独设置材质"),Margin=new Thickness(0,8,0,8) };
    ...
    public ThemeCenterWindow(App app, Window owner, AppCategory category = null)
    {
        ...
        _app = app; Owner = owner; Title = AppLanguage.T("主题中心"); Icon = new System.Windows.Media.Imaging.BitmapImage(new Uri("pack://application:,,,/Assets/app.ico"));
        ...
        scopeMark.SetValue(TextBlock.TextProperty,AppLanguage.T("（已自定义）")); scopeMark.SetValue(TextBlock.MarginProperty,new Thickness(6,0,0,0));
        ...
        var heading = new StackPanel(); heading.Children.Add(Label(AppLanguage.T("主题中心"),24)); heading.Children.Add(Label(AppLanguage.T("让每个分类拥有自己的氛围。修改后点击应用保存。"),12)); header.Children.Add(heading);
        ...
        left.Children.Add(Label(AppLanguage.T("应用范围"),15));
        _scope.Items.Add(new ScopeName(AppLanguage.T("全局默认"))); foreach (var cat in app.Config.Categories) _scope.Items.Add(cat);
        left.Children.Add(_scope); left.Children.Add(_colors);
        BuildPresetControls(left);
        left.Children.Add(Label(AppLanguage.T("预设配色"),15));
        var presets = new WrapPanel();
        foreach (var preset in ThemeService.Presets)
        {
            var button = Button(AppLanguage.T(preset.Name),()=>
            {
        ...
        left.Children.Add(presets); left.Children.Add(Label(AppLanguage.T("窗口材质"),15)); left.Children.Add(_materialOverride);
        foreach(var (value,title) in new[] { ("Solid",AppLanguage.T("普通")),("Frosted",AppLanguage.T("磨砂玻璃")) })
        {
            var material=value;
        ...
        left.Children.Add(Label(AppLanguage.T("分类可独立设置配色或背景；材质也可独立设置；取消勾选即跟随全局。"),11));
        ...
        _previewTitle.Text=AppLanguage.T("实时预览"); _previewTitle.FontSize=15; _previewTitle.Margin=new Thickness(0,8,0,8); right.Children.Add(_previewTitle); right.Children.Add(_previewCard);
        ...
        var demoTitle = new TextBlock { Text = AppLanguage.T("启动台   ·   我的分类"), FontSize = 15, FontWeight = FontWeights.SemiBold, Margin = new Thickness(0,0,0,14) }; demoTitle.SetResourceReference(TextBlock.ForegroundProperty,"TextPrimaryBrush"); demo.Children.Add(demoTitle);
        var icons = new UniformGridCompat();
        foreach (string name in new[] { AppLanguage.T("工作"), AppLanguage.T("文件"), AppLanguage.T("照片"), AppLanguage.T("音乐") })
        {
 ...
 tile.Children.Add(glyph);
        ...
        _backgroundSection.Children.Add(_background); _backgroundSection.Children.Add(Label(AppLanguage.T("背景素材"),15));
        var actions = _backgroundActions; actions.Children.Add(Button(AppLanguage.T("上传背景…"),PickBackground)); actions.Children.Add(Button(AppLanguage.T("清除背景"),()=> { _draft.BackgroundPath=null; _draft.OverrideBackground=true; LoadEditors(); RefreshPreview(); })); _backgroundSection.Children.Add(actions);
        ...
        percent.SetResourceReference(TextBlock.ForegroundProperty,"AccentDeepBrush"); DockPanel.SetDock(percent,Dock.Right); opacityHeader.Children.Add(percent); opacityHeader.Children.Add(Label(AppLanguage.T("背景遮罩"),12));
        opacityPanel.Children.Add(opacityHeader);
        _dim.Style=(Style)FindResource("OpacitySlider"); opacityPanel.Children.Add(_dim);
        var ends=new DockPanel(); var solid=Label(AppLanguage.T("柔和易读"),11); DockPanel.SetDock(solid,Dock.Right); ends.Children.Add(solid); ends.Children.Add(Label(AppLanguage.T("清晰通透"),11)); opacityPanel.Children.Add(ends);
        ...
        glassPercent.SetResourceReference(TextBlock.ForegroundProperty,"AccentDeepBrush"); DockPanel.SetDock(glassPercent,Dock.Right); glassHeader.Children.Add(glassPercent); glassHeader.Children.Add(Label(AppLanguage.T("磨砂强度"),12));
        ...
        var glassEnds=new DockPanel(); var glassRight=Label(AppLanguage.T("深度磨砂"),11); DockPanel.SetDock(glassRight,Dock.Right); glassEnds.Children.Add(glassRight); glassEnds.Children.Add(Label(AppLanguage.T("清晰透明"),11)); glassPanel.Children.Add(glassEnds);
        ...
        depthHeader.Children.Add(depthPercent); depthHeader.Children.Add(Label(AppLanguage.T("玻璃颜色深度"),12)); depthPanel.Children.Add(depthHeader);
        _glassColorDepth.Style=(Style)FindResource("OpacitySlider"); depthPanel.Children.Add(_glassColorDepth);
        var depthEnds=new DockPanel(); var depthRight=Label(AppLanguage.T("不透明底色"),11); DockPanel.SetDock(depthRight,Dock.Right);
        depthEnds.Children.Add(depthRight); depthEnds.Children.Add(Label(AppLanguage.T("无染色"),11)); depthPanel.Children.Add(depthEnds);
        depthPanel.Children.Add(Label(AppLanguage.T("使用当前配色的面板底色，不影响磨砂强度。"),11));
        _glassSection.Children.Add(depthCard);
        _glassSection.Children.Insert(0,Button(AppLanguage.T("恢复磨砂默认配置（70%）"),()=> { EnsureMaterialOverride(); _draft.GlassBlurStrength=.7; _draft.GlassColorDepth=.7; LoadEditors(); RefreshPreview(); }));
        ...
        var saveClose = Button(AppLanguage.T("保存并关闭"),async ()=> { if (await SaveAsync()) Close(); }); saveClose.SetResourceReference(BackgroundProperty,"TabActiveBgBrush"); saveClose.SetResourceReference(ForegroundProperty,"TabActiveTextBrush"); DockPanel.SetDock(saveClose,Dock.Right); footer.Children.Add(saveClose);
        var apply = Button(AppLanguage.T("应用并保存"),async ()=>await SaveAsync()); DockPanel.SetDock(apply,Dock.Right); footer.Children.Add(apply);
        var cancel = Button(AppLanguage.T("取消"),()=>Close()); DockPanel.SetDock(cancel,Dock.Right); footer.Children.Add(cancel);
        var restore=Button(AppLanguage.T("恢复跟随全局"),()=> { if (Category != null) { _draft = ThemeService.Global(app.Config); _draft.OverrideColors=false; _draft.OverrideBackground=false; _draft.OverrideMaterial=false; _draft.OverrideIcons=false; LoadEditors(); RefreshPreview(); } }); restore.HorizontalAlignment=HorizontalAlignment.Left; footer.Children.Add(restore);
        var resetAll=Button(AppLanguage.T("恢复默认主题"),ResetDefaultTheme); resetAll.HorizontalAlignment=HorizontalAlignment.Left; footer.Children.Add(resetAll);
    ...
    {
        if (!PromptDialog.Confirm(this, AppLanguage.T("恢复默认主题"), "确定要恢复默认主题吗？\n全局配色、背景、材质、图标外观以及各分类的独立主题配置将全部回归默认。"))
            return;
        ...
        LoadEditors(); RefreshPreview();
        _status.Text = AppLanguage.T("已恢复默认主题并保存。");
    }
    ...
    private void LoadEditors()
    {
        ...
        _materialHint.Text=material switch { "Frosted"=>AppLanguage.T("模糊窗口后方的桌面与其他窗口，叠加细腻磨砂。启用后停用自定义背景。"),_=>AppLanguage.T("使用主题配色，可添加图片、动图或视频背景。") };
        _file.Text=string.IsNullOrEmpty(_draft.BackgroundPath)?AppLanguage.T("未设置背景 · 使用主题底色"):Path.GetFileName(_draft.BackgroundPath);
        foreach (var pair in _swatches)
        {
            string value=pair.Key=="Surface"?_draft.Surface:pair.Key=="Text"?_draft.Text:_draft.Accent;
            pair.Value.Content=$"{(pair.Key=="Surface"?AppLanguage.T("面板底色"):pair.Key=="Text"?AppLanguage.T("文字颜色"):AppLanguage.T("强调色"))}    {value}";
        }
    ...
    private void RefreshPreview()
    {
        ...
        _file.Text=glass?AppLanguage.T("背景已停用")+(string.IsNullOrEmpty(_draft.BackgroundPath)?"":AppLanguage.T(" · 原背景已保留")):string.IsNullOrEmpty(_draft.BackgroundPath)?AppLanguage.T("未设置背景 · 使用主题底色"):Path.GetFileName(_draft.BackgroundPath);
        _previewTitle.Text=glass?AppLanguage.T("实时窗口材质预览（可拖动窗口查看）"):AppLanguage.T("实时预览");
        var tint=ThemeService.GlassSurfaceColor(effective);
        _surfaceCard.Background=new SolidColorBrush(tint);
        if(!_windowMaterial.Apply(effective,0) && IsLoaded) _status.Text=AppLanguage.T("当前系统无法启用窗口玻璃效果。");
    ...
    private void PickColor(string key)
    {
        ...
        _draft.Name=AppLanguage.T("自定义"); _draft.OverrideColors=true; LoadEditors(); RefreshPreview();
    }
    ...
    private void PickBackground()
    {
        ...
        var dialog=new Microsoft.Win32.OpenFileDialog { Title=AppLanguage.T("选择背景素材"), Filter=AppLanguage.T("背景素材")+"|*.png;*.jpg;*.jpeg;*.bmp;*.gif;*.mp4;*.wmv;*.avi;*.mov;*.m4v" };
    ...
    private async Task<bool> SaveAsync()
    {
        ...
        {
            await _previewTask;
            if (_previewFailed) { _status.Text=AppLanguage.T("请更换有效背景或清除背景后再保存。"); return false; }
            ...
            var previous=Category==null?_app.Config.GlobalTheme:Category.ThemeOverride;
            ...
            if(!ConfigService.Save(_app.Config))
            {
                if(Category==null) _app.Config.GlobalTheme=previous; else Category.ThemeOverride=previous;
                _status.Text=AppLanguage.T("保存失败，请检查文件是否可用及磁盘空间。"); return false;
            }
            _app.ApplyTheme(_app.Config.Theme); _app.RefreshMainTheme();
            _draft=saved; _status.Text=AppLanguage.T("已应用并保存");
            return true;
        }
        catch(Exception) { _status.Text=AppLanguage.T("保存失败，请检查文件是否可用及磁盘空间。"); return false; }
